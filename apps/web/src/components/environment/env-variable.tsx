...
export default function EnvVariables({
  ...
  const content = (
    <div className="space-y-4">
      <div
        className="grid grid-cols-[1fr,2fr,auto] items-start gap-4"
        ref={animationRef}
      >
        ...
        {envVars.map((envVar, index) => (
          <React.Fragment key={index}>
            <Input
              ...
              className="font-mono bg-black/20 border-white/10 focus:border-violet-500/50"
      ...
      <Button
        ...
        variant="outline"
        onClick={addMore}
        className="flex items-center gap-2 bg-white/5 border-white/10 hover:bg-white/10 hover:text-white"
      >
  ...
  return (
    <div
      className={cn("w-full", {
        "rounded-lg border border-white/10 bg-white/5 p-4": mode === "deploy",
      })}
      onPaste={handlePaste}
    >
      {mode === "deploy" ? (
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          <CollapsibleTrigger className="flex w-full items-center text-lg font-medium hover:text-white transition-colors">
            <ChevronDown
